...
const calculateTotalBalance = users => users.reduce((balanceSum, user) => balanceSum + user.balance, 0)
...
const getNamesSortedByFriendsCount = users => users.sort(user => user.friends.length).map(user => user.name);
...
const getSortedUniqueSkills = users => users.map(user => user.skills).reduce((aсс, items) => [...aсс, ...items], []).reduce((uniq, item) => uniq.includes(item) ? uniq : [...uniq, item], []).sort();
